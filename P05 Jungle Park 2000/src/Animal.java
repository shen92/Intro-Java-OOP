...
public class Animal implements ParkGUI {
  ...
  public void draw() {
    ...
    // display label
    displayLabel();
    // implement action of the animal
    action();
  ...
  /**
   * Check if another animal is in its neighborhood
   * 
   * @param otherAnimal reference to another animal
   * @param range the range to check
   * @return if another animal is in its neighborhood
   */
  public boolean isClose(Animal otherAnimal, int range) {
    if (this.distance(otherAnimal) <= range)
      return true;// return true if the animal is in range
    return false;
  }
